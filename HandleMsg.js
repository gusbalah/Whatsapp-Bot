...
module.exports = HandleMsg = async (pakforlay, message) => {
    try {
        ...
        case 'ownerbot':
            ...
            .then(() => pakforlay.sendText(from, 'Jika kalian ingin request fitur silahkan chat nomor admin!\n\nMohon Donasi seiklas nya untuk isi kuota, karna umur bot bergantung dengan kuota admin\n\n*KLIK LAMAN DIBAWAH INI UNTUK BERDONASI*\n\n*LINK AMAN BUKAN SCAM/PHISING*\n\n*https://saweria.co/pakforlay*'))
            break
        case 'join':
          ...
          //  if (args.length >= 2){
            //    const BrainlySearch = require('./lib/brainly')
              //  let tanya = body.slice(9)
               ...
               // if (jum > 10) return pakforlay.reply(from, 'Max 10!', id)
               // if (Number(tanya[tanya.length-1])){
                //    tanya
               // }
               // pakforlay.reply(from, `➸ *Pertanyaan* : ${tanya.split('.')[0]}\n\n➸ *Jumlah jawaban* : ${Number(jum)}`, id)
               // await BrainlySearch(tanya.split('.')[0],Number(jum), function(res){
                 //   res.forEach(x=>{
                   //     if (x.jawaban.fotoJawaban.length == 0) {
                     //       pakforlay.reply(from, `➸ *Pertanyaan* : ${x.pertanyaan}\n\n➸ *Jawaban* : ${x.jawaban.judulJawaban}\n`, id)
                      // } else {
        ...
        case 'images':
            ...
            pakforlay.sendFileFromUrl(from, hasilwall, '', '', id)
            .catch(() => {
                pakforlay.reply(from, 'Ada yang eror!', id)
            })
            break
        case 'sreddit':
            ...
            pakforlay.sendFileFromUrl(from, hasilreddit, '', '', id)
            .catch(() => {
                pakforlay.reply(from, 'Ada yang eror!', id)
            })
        case 'resep':
            ...
            pakforlay.reply(from, hasilresep + '\n\nIni kak resep makanannya..', id)
            .catch(() => {
                pakforlay.reply(from, 'Ada yang eror!', id)
            })
            break
        case 'nekopoi':
            pakforlay.sendText(from, `Sedang mencari video terbaru dari website nekopoi...`)
            rugapoi.getLatest()
            .then((result) => {
        ...
        case 'chord':
            ...
            break
		case 'getses':
            const sesPic = await pakforlay.getSnapshot()
            pakforlay.sendFile(from, sesPic, 'session.png', 'Neh...', id)
            break
        ...
        case 'bc': //untuk broadcast atau promosi
            ...
            for (let idk of chatz) {
                ...
                if (!cvk.isReadOnly) pakforlay.sendText(idk, `〘 *PakForlay BOT* 〙\n\n${msg}`)
                if (cvk.isReadOnly) pakforlay.sendText(idk, `〘 *PakForlay* 〙\n\n${msg}`)
            }
